...
m_Chl3ySSS2 = gls(model = riqueza ~ poly(Chl, 3) + poly(SSS , 2)  ,data = datos, weights = varIdent(form = ~1|source),correlation = corCAR1(form = ~ dist|source))
...
m_SSS2xgamma = gls(model = riqueza ~ poly(SSS, 2) * gamma   ,data = datos, weights = varIdent(form = ~1|source),correlation = corCAR1(form = ~ dist|source))

m_SSSxgamma = gls(model = riqueza ~ SSS * gamma   ,data = datos, weights = varIdent(form = ~1|source),correlation = corCAR1(form = ~ dist|source))

m_SSS2_sinMatriz = gls(model = riqueza ~ scale(SSS) + I(scale(SSS)^2),
             data = datos, 
             weights = varIdent(form = ~1|source))


m_gamma_sinMatriz= gls(model = riqueza ~ scale(gamma),
             data = datos, 
             weights = varIdent(form = ~1|source))
...
modelos_sinSource <- list(
  ...
  "m_Chl3 + SSS2" = m_Chl3ySSS2,
  "Chl * SSS" = m_ChlxSSS,
  "gamma^2" = m_gamma2,
  "gamma^3" = m_gamma3,
  "SSS2 * gamma" = m_SSS2xgamma,
  "m_SSSxgamma" = m_SSSxgamma,
  "m_SSS2_sinMatriz" = m_SSS2_sinMatriz,
  "m_gamma_sinMatriz" = m_gamma_sinMatriz
)
...
### Volviendo al Poisson

Considerando sin matriz de correlación porque datos son practicamente independientes

datos\$dist\[51\] = 0.001

```{r}
library(glmmTMB)
modelo_3<-glmmTMB(riqueza ~ SSS + SST + POC  + gamma,
                  family = compois,
                  data = datos)
  ...
  simRes <- simulateResiduals(fittedModel = modelo_3)
  
  
testSpatialAutocorrelation(
  simulationOutput = simRes,
  x = datos_crudos$lat,     # Tu columna de coordenada X o Longitud
  y = datos_crudos$lon,     # Tu columna de coordenada Y o Latitud
  plot = TRUE
)


indices_cero <- which(datos$dist == 0)
datos$dist[1] <- 0.0001

testTemporalAutocorrelation(
  simulationOutput = simRes,
  time = datos$dist,  # Usamos 'dist' como el vector de ordenamiento
  plot = TRUE
)
```

### Modelos Ditancia alternativa
...
ref_points <- data.frame(
  source = c("bloom", "talud"),
  ref_lat = c(-48.2656, -48.2656),
  ref_lon = c(-63.7595, -63.7595)
)
...
datos_alternativos = datos_crudos[, -c(2,3,12:14 )]


modelo_3<-glmmTMB(riqueza ~ SSS + SST + POC  + gamma,
                  family = compois,
                  data = datos_alternativos)
...
testTemporalAutocorrelation(
  simulationOutput = simRes,
  time = datos_alternativos$dist,  # Usamos 'dist' como el vector de ordenamiento
  plot = TRUE
)
...
m_Nulo_alt = gls(model = riqueza ~ 1 , weights = varIdent(form = ~1|source) , data = datos_alternativos,correlation = corCAR1(form = ~ dist|source))

m_gamma_alt =gls(model = riqueza ~ gamma   ,data = datos_alternativos, weights = varIdent(form = ~1|source),correlation = corCAR1(form = ~ dist|source))

m_SSS2_alt = gls(model = riqueza ~ scale(SSS) + I(scale(SSS)^2),
             data = datos_alternativos, 
             weights = varIdent(form = ~1|source) ,correlation = corCAR1(form = ~ dist|source))


m_SSS2_sinMatriz_alt = gls(model = riqueza ~ scale(SSS) + I(scale(SSS)^2),
             data = datos_alternativos, 
             weights = varIdent(form = ~1|source))


m_gamma_sinMatriz_alt= gls(model = riqueza ~ scale(gamma),
             data = datos_alternativos, 
             weights = varIdent(form = ~1|source) ,correlation = corCAR1(form = ~ dist|source))


modelos_alternativos <- list(
  "Nulo" = m_Nulo,
  "Nulo alt" = m_Nulo_alt,
  "gamma" = m_gamma,
  "gamma alt" = m_gamma_alt,
  "m_gamma_sinMatriz" = m_gamma_sinMatriz,
  "SSS^2" = m_SSS2,
  "SSS^2 alt" = m_SSS2_alt,
  "m_SSS2_sinMatriz" = m_SSS2_sinMatriz,
  "m_SSS2_sinMatriz_alt" = m_SSS2_sinMatriz_alt
)
...
```{r}
aic_values <- sapply(modelos_alternativos, AIC)
bic_values <- sapply(modelos_alternativos, BIC)
rse_values <- sapply(modelos_alternativos, sigma)
phi_values <- sapply(modelos_alternativos, function(m) {
...
p_value_last_coef <- sapply(modelos_alternativos, function(m) {
...
#| fig-width: 6
#| fig-height: 5
model_names <- names(modelos_alternativos)
# sapply(modelos_sinSource, check_model) 

for (current_name in model_names) {
  
  m <- modelos_alternativos[[current_name]]
...
aic_values <- sapply(modelos_finales, AIC)
bic_values <- sapply(modelos_finales, BIC)
rse_values <- sapply(modelos_finales, sigma)
phi_values <- sapply(modelos_finales, function(m) {
...
p_value_last_coef <- sapply(modelos_finales, function(m) {
...
model_names <- names(modelos_finales)

for (current_name in model_names) {
  
  m <- modelos_finales[[current_name]]
...
datos$riqueza = as.numeric(datos$riqueza)
modelo_final =   gls(model = riqueza ~ scale(Chl)+ I(scale(Chl)^2) + I(scale(Chl)^3) +
             scale(SSS) + I(scale(SSS)^2) +
             scale(Chl):scale(SSS) ,data = datos, weights = varIdent(form = ~1|source),correlation = corCAR1(form = ~ dist|source) )
  
  
summary(modelo_final)

pred_salinidad <- ggpredict(update(modelo_final,correlation = NULL), terms = c("SSS [all]", "Chl [5,18]"))
plot(pred_salinidad)

pred_salinidad <- ggpredict(update(modelo_final,correlation = NULL), terms = c("SSS [all]", "Chl [5,18]"))
plot(pred_salinidad)

pred_Chl <- ggpredict(update(modelo_final,correlation = NULL), terms = c( "Chl [all]", "SSS [33.2,33.9]"))
plot(pred_Chl)


pred_salinidad <- ggpredict(update(m_Chl3ySSS2,correlation = NULL), terms = c("SSS [all]", "Chl [5,18]"))
plot(pred_salinidad)
...
ggplot(datos, aes(x=SSS, y= Chl)) + 
  geom_point(  size=3,  shape=19 ) +  
  xlab( "SSS" ) +  ylab("Chl") 


ggplot(datos, aes(x=SSS, y= gamma)) + 
  geom_point(  size=3,  shape=19 ) +  
  xlab( "SSS" ) +  ylab("gamma") 

  
  ggplot(datos, aes(x=SSS, y= bbp)) + 
  geom_point(  size=3,  shape=19 ) +  
  xlab( "SSS" ) +  ylab("bbp") 

  
ggplot(datos, aes(x=SSS, y= POC)) + 
  geom_point(  size=3,  shape=19 ) +  
  xlab( "SSS" ) +  ylab("POC") 

  
ggplot(datos, aes(x=SSS, y= SST)) + 
  geom_point(  size=3,  shape=19 ) +  
  xlab( "SSS" ) +  ylab("SST") 
...
min = min(pred_salinidad$predicted)

pred_salinidad [which(pred_salinidad$predicted ==  min)]




m_raw <- gls(
  riqueza ~ poly(SSS, 2, raw = TRUE),
  data = datos,
  weights = varIdent(form = ~1|source),
  correlation = corCAR1(form = ~ distm|source)
)

summary(m_raw)

summary(m_SSS2_sinMatriz)
```
