...
m_Nulo = gls(model = riqueza ~ 1 ,data = datos, weights = varIdent(form = ~1|source),correlation = corCAR1(form = ~ dist|source))
...
```{r}
m_interaccionTotal =gls(model = riqueza ~ bbp  * source * SSS  * SST  * gamma    ,data = datos, weights = varIdent(form = ~1|source) ,correlation = corCAR1(form = ~ dist|source))

AIC(m_Nulo, m_Source , m_SSS , m_Clo , m_Clo_S , m_SSS_S,m_POC , m_POC_S  ,m_bbp, m_bbp_S ,m_dist , m_dist_S , m_dist_Sxbbp, m_distxbbp,m_gamma , m_gamma_S, m_SSS_Sxbbp , m_interaccionTotal)
BIC(m_Nulo, m_Source , m_SSS , m_Clo , m_Clo_S , m_SSS_S,m_POC , m_POC_S  ,m_bbp, m_bbp_S ,m_dist , m_dist_S , m_dist_Sxbbp, m_distxbbp , m_gamma , m_gamma_S, m_SSS_Sxbbp, m_interaccionTotal)
summary(m_interaccionTotal)
...
ggplot(datos, aes(x=riqueza, y=Chl , group = source , color = source)) + 
  geom_point(size=3,  shape=19 ) +  
  geom_smooth(method=lm, se=F, fullrange=F, size=0.5)+ 
  xlab("Riqueza") +  ylab("Chl") 

ggplot(datos, aes(x=riqueza, y=bbp , group = source , color = source)) + 
  geom_point(size=3,  shape=19 ) +  
  geom_smooth(method=lm, se=F, fullrange=F, size=0.5)+ 
  xlab("Riqueza") +  ylab("bbp") 

ggplot(datos, aes(x=gamma, y=bbp , group = source , color = source)) + 
  geom_point(size=3,  shape=19 ) +  
  geom_smooth(method=lm, se=F, fullrange=F, size=0.5)+ 
  xlab("Riqueza") +  ylab("gamma") 
...
```{r}
modelo_N_VarIdent<-gls(model = riqueza ~ Chl + SSS + source + gamma + bbp + POC + SST,data = datos, weights = varIdent(form = ~1|source),correlation = corCAR1(form = ~ dist|source))

modelo_N_VarPower = update(modelo_N_VarIdent,  weights = varPower())
```
